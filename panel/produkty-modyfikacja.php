...
<!-- Wybierz klienta po id -->
	<div class="col-md-6">
			...
			<div class="border border-dark rounded m-4 p-3">
				<form action="klienci-modyfikacja.php" method="post">
					<div class="form-group p-3">
						<input class="form-control" type="text" placeholder="Podaj kod produktu, aby wyswietlic produkt" name="kod_produkt_szukaj">	
					</div>
		...
		<div class="obecne-dane-produkt m-4 p-3">
			<?php
    			if(isset($_POST["kod_produkt_szukaj"])) {
        		$kod_produkt = $_POST["kod_produkt_szukaj"];
         
        	require('db_data.php');
         
        	$zapytanie_wszystkie_produkty = "SELECT pb.kod_produktu, pb.producent, pb.cena, kategoria.typ_kategoria From kategoria AS pb Join parametry_baterie on pb.id_kategoria= kategoria.id_kategoria UNION SELECT pc.kod_produktu, pc.producent, pc.cena, kategoria.typ_kategoria From kategoria AS pc Join parametry_czujniki on pc.id_kategoria= kategoria.id_kategoria UNION SELECT pd.kod_produktu, pd.producent, pd.cena, kategoria.typ_kategoria From kategoria AS pd Join parametry_diody on parametry_diody.id_kategoria= kategoria.id_kategoria UNION SELECT pk.kod_produktu, pk.producent, pk.cena, kategoria.typ_kategoria From kategoria AS pk Join parametry_kondensatory on pk.id_kategoria= kategoria.id_kategoria";
	             
	            $zapytanie_wszystkie_produkty = $connect->query($zapytanie_wszystkie_produkty);
